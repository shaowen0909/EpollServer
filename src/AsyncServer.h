...
public:
    ...
    [[noreturn]] void EventLoop()
	{
		while(true)
		{
			...
			if((last_socket_check_ + 1) < time(nullptr) && !clients_.empty())
			{
				auto it = clients_.begin();
				while(it != clients_.end()) 
				{
                    ExampleClient* client = (*it).second;
				...
				last_socket_check_ = time(nullptr);
			}
		}
	}
...
private:
	...
	bool HandleAccept()
	{
		...
		client = new ExampleClient(client_fd,
                                          client_sin.sin_addr,
	...
	bool HandleClient(epoll_event ev)
	{
		...
		auto *client = reinterpret_cast<ExampleClient *>(ev.data.ptr);
	...
	void RemoveClient(ExampleClient* client)
	{
	    // std::map<int, ClientDescriptor *>::iterator it = clients_.find(client->uid());
		auto it = clients_.find(client->uid());
		clients_.erase(it);
	}
...
private:
    ...
    epoll_event* events_;
    std::map<int, ExampleClient*> clients_;
};